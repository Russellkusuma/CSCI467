...
def train_gradient_descent(X_train, y_train, lr=1e-2, num_iters=400):
    ...
    for i in range(num_iters):
        #y_pred = w*X_train # The current predicted value of Y
        y_pred = predict(w, X_train)
        total_gradient = (2)*(X_train.T * (y_pred - y_train))
        gradient = np.mean(total_gradient, axis = 1)
...
def train_normal_equations(X_train, y_train):
    ...
    """
    ### BEGIN_SOLUTION 3b
    # Finding dot of x and x.T
    X_T_X = X_train.T.dot(X_train)
    # Inverse the matrix
    inverse = np.linalg.pinv(X_T_X)
    # Dot prod of x.T and y
    X_T_y = X_train.T.dot(y_train)
    return inverse.dot(X_T_y)
    ### END_SOLUTION 3b
